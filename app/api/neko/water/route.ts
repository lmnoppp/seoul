...
export async function POST(request: NextRequest) {
  try {
    ...
    // Récupère l'état actuel et met à jour thirst +40 (cap 100)
    ...
    const newThirst = Math.min((currentState?.thirst || 0) + 40, 100)
    ...
    if (updateError) {
      console.error('Erreur update neko thirst:', updateError)
      return NextResponse.json({ error: 'Erreur mise à jour neko' }, { status: 500 })
    }

    return NextResponse.json(state)
  } catch (error) {
    console.error('Erreur POST /api/neko/water:', error)
